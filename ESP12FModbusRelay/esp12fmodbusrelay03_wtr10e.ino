...
ModbusMaster node1;
ModbusMaster node2;       //WTR10-E Temperature&Humidity
//===================Modbus=================//
...
//=====Begin config Blynk & Set time interval=====//
  ...
  timer.setInterval(5000L, wtr10e01);
}
//=====Setup Function=====//
...
void wtr10e01(){
  uint8_t result1;   
  float temp = (node2.getResponseBuffer(0)/10.0f);
  float humi = (node2.getResponseBuffer(1)/10.0f);
 
  Serial.println("Get WTR10E-01 Data1");
  result1 = node2.readHoldingRegisters(0x0000, 2); // Read 2 registers starting at 1)
...
//****BUTTON ON/OFF Valve1****//
 BLYNK_WRITE(Widget_Btn_Valve1){
      ...
      if(valueValve1 == 1){    
        //Modbus command to ON/OFF Relay           
        pool_size1 = node1.writeSingleRegister(0x05,0x0100);    //Turn on Valve1
        pool_size1 = node1.writeSingleRegister(0x01,0x0100);    //Turn on Pump
      }
       else{                    
       pool_size1 = node1.writeSingleRegister(0x05,0x0200);     //Turn off Valve1
       pool_size1 = node1.writeSingleRegister(0x01,0x0200);     //Turn off Pump
     }
} 
//****BUTTON ON/OFF Valve1****//
...
//****BUTTON ON/OFF Valve2****//
 BLYNK_WRITE(Widget_Btn_Valve2){
      ...
      if(valueValve2 == 1){    
        //Modbus command to ON/OFF Relay           
        pool_size1 = node1.writeSingleRegister(0x06,0x0100);    //Turn on Valve2
        pool_size1 = node1.writeSingleRegister(0x01,0x0100);    //Turn on Pump
      }
       else{                    
       pool_size1 = node1.writeSingleRegister(0x06,0x0200);     //Turn off Valve2
       pool_size1 = node1.writeSingleRegister(0x01,0x0200);     //Turn off Pump
     }
} 
//****BUTTON ON/OFF Valve2****//
...
//****BUTTON ON/OFF Valve3****//
 BLYNK_WRITE(Widget_Btn_Valve3){
      ...
      if(valueValve3 == 1){    
        //Modbus command to ON/OFF Relay           
        pool_size1 = node1.writeSingleRegister(0x07,0x0100);    //Turn on Valve3
        pool_size1 = node1.writeSingleRegister(0x01,0x0100);    //Turn on Pump
      }
       else{                    
       pool_size1 = node1.writeSingleRegister(0x07,0x0200);     //Turn off Valve3
       pool_size1 = node1.writeSingleRegister(0x01,0x0200);     //Turn off Pump
     }
} 
//****BUTTON ON/OFF Valve3****//
...
//****BUTTON ON/OFF Valve4****//
 BLYNK_WRITE(Widget_Btn_Valve4){
      ...
      if(valueValve4 == 1){    
        //Modbus command to ON/OFF Relay           
        pool_size1 = node1.writeSingleRegister(0x08,0x0100);    //Turn on Valve4 
        pool_size1 = node1.writeSingleRegister(0x01,0x0100);    //Turn on Pump
      }
       else{                    
       pool_size1 = node1.writeSingleRegister(0x08,0x0200);     //Turn off Valve4
       pool_size1 = node1.writeSingleRegister(0x01,0x0200);     //Turn off Pump
     }
} 
//****BUTTON ON/OFF Valve4****//
