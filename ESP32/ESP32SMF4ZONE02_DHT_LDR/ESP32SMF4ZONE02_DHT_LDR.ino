...
//==========LDR1============//
//V13 ปุ่มสถานะเปิดปิดไฟ1
//V14 ปุ่มเปิดปิดไฟ1
//V32 LDR1
//V61 Auto&Manual LDR1
//V62 Slider LDR1
//==========LDR1============//
...
#define ledlight        12              //LDR turn on Light
...
//V61 Auto&Manual LDR1
//V62 Slider LDR1
#define Widget_LED_LDR1 V13               //ไฟสถานะปุ่มไฟ1
#define Widget_Btn_LDR1 V14               //ปุ่ม เปิด-ปิดไฟ1
WidgetLED LedBlynkLDR1(Widget_LED_LDR1);
bool switchStatus6 = 0;                   // 0 = manual,1=auto
int LDRsensorLimit6 = 0;
...
void setup() {
 ...
 Serial.begin(9600);  
  ...
  //wifiManager.resetSettings(); //ถ้าต้องการ Reset ค่าการเชื่อมต่อ Wi-Fi หรือ Blynk Token ให้เอา Comment บรรทัดนี้ออก

  //ถ้าจะทำปุ่ม Reset ค่าการเชื่อมต่อ Wi-Fi หรือ Blynk Token ให้เอา Comment จาก บรรทัดที่ 326 - 352 ออก 
  //และประกาศตัวแปร เพื่อรับค่าการกดปุ่ม เช่น  #define AP_Config 17 
  /*
  ...
  */
  ...
  //บรรทัดที่ 370 - 375 ถ้าไม่สามารถเชื่อมต่อ Wi-Fi เดิม หรือ Wi-Fi ที่เคยเชื่อมต่อก่อนหน้านี้ได้ ให้ทำการปรับโหมดการทำงานของ
  //Board ให้กลับมาเป็น Access Point เพื่อที่เราจะได้ทำการใช้โทรศัพท์มือถือ หรือ Notebook เชื่อมต่อมาที่ Board
  //เพื่อทำการกำหนดค่าการเชื่อมต่อเครือข่ายอินเตอร์เน็ต 
  //แต่ถ้าหากหาชื่อ Wi-Fi เดิมที่เคยเชื่อมต่อก่อนหน้านี้ ให้ทำกระบวนการเชื่อมต่อ Wi-Fi และ เชื่อมต่อไปที่ Blynk Server 
  if (!wifiManager.autoConnect("SMF2HP4ZONE","password")) {
    Serial.println("failed to connect and hit timeout");//แสดงข้อความใน Serial Monitor
    delay(100);
    //reset and try again, or maybe put it to deep sleep
    ESP.restart();//แก้ เดิม ESP.reset(); //ถ้าไม่กระทำการใดๆ ภายในเวลา 2 นาที ตัว Board จะ Reset และเริ่มกระบวนการทำงานใหม่
  }
  ...
  timer.setInterval(1000L, checkphysic_btn_state);    //Check สถานะการกดปุ่ม
  timer.setInterval(5000L,dhtSensorData1);            //อ่านค่าอุณหภูมิ ความชื้นทุกๆ 5 วินาที
  timer.setInterval(5000L,LDR1);                      //อ่านค่าแสงสว่างทุกๆ 5 วินาที                   
  timer.setInterval(3000L, getSoilMoisterData1);      //อ่านค่าความชื้นในดินจุดที่ 1 ทุกๆ 3 วินาที
  timer.setInterval(3000L, getSoilMoisterData2);      //อ่านค่าความชื้นในดินจุดที่ 2 ทุกๆ 3 วินาที
  timer.setInterval(3000L, getSoilMoisterData3);      //อ่านค่าความชื้นในดินจุดที่ 3 ทุกๆ 3 วินาที
  timer.setInterval(3000L, getSoilMoisterData4);      //อ่านค่าความชื้นในดินจุดที่ 4 ทุกๆ 3 วินาที 
  timer.setInterval(1000L, clockDisplay);             //แสดงวันที่ เวลา
...
void reconnectblynk()
{
  ...
  //นับจำนวนการเชื่อมต่อ Server blynk ไม่ได้ หากเกินหรือเท่ากับ 5 ให้ Reset Board และเริ่มกระบวนการทำงานใหม่
...
//ถ้าใช้ปั้มน้ำออโต้ ปุ่มสำหรับเปิดปิดปั้มก็ไม่จำเป็นต้องมี
//=====BUTTON ON/OFF Pump========//
...
BLYNK_WRITE(V62)
{   
  LDRsensorLimit6 = param.asInt(); // Get value as integer
}
//=====Update LDR1 setting=====//
...
void LDR1(){
  ...
  Blynk.virtualWrite(V32,(ldr_percentage1)); 
  ...
  {
    // auto
    if(ldr_percentage1 < LDRsensorLimit6)
    {
