...
//*****Blynk Virtual Pin*****//
//Virtual Pin ควบคุมการเปิดปิด
//V1  ปุ่ม เปิด-ปิด พัดลมระบายอากาศ
//V2  ปุ่ม เปิด-ปิด ปั้มน้ำ
//V3  ปุ่ม เปิด-ปิด เครื่องพ่นหมอกอัลตร้าโซนิค
//Virtual Pin รับค่าจาก Sensor
//V4  WindSpeed
//V5  SoilMoisture  
//V6  Temperature
//V7  Humidity
...
void setup()
{
  ...
  timer.setInterval(1000L, WindSpeed);
  timer.setInterval(1100L, SoilMoisture);
  timer.setInterval(1200L, DL13_Temp_Humi);
}
//=====Setup Function=====//
...
BLYNK_WRITE(V12)
{   
  WindSpeddLevelLimit1 = param.asInt(); // Get value as integer
}
...
{
  uint8_t result;  
  float windspeed = (node1.readHoldingRegisters(0x0000, 1)); // Read 2 registers starting at 1)
  ...
  {
    // auto
    if(windspeed < WindSpeddLevelLimit1)
    {
...
{
  uint8_t result;
  float SoilMoisture = (node2.readHoldingRegisters(0x0000, 3));
  ...
  {
    // auto
    if(SoilMoisture < SoilLevelLimit2)
    {
