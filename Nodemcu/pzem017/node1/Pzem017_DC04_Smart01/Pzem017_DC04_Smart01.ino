...
WiFiClient Solar_Node1_IN;      //ถ้าเชื่อมต่อเข้า MQTT Server หลายโหนดพร้อมกันให้เปลี่ยยนชื่อให้ไม่เหมือนกัน
PubSubClient client(Solar_Node1_IN);
...
//if (WiFi.SSID()!="") wifiManager.setConfigPortalTimeout(60);
  ...
  if (!wifiManager.autoConnect("Solar_Node1_IN", "password")) {
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("Solar_Node1_IN")) {
...
client.publish("Solar_Node1_IN/voltagein", voltagein); //Publish message to MQTT Topic Solar_Node1_IN/voltagein

static char currentin[7]; 
dtostrf (PZEMCurrent,6,2,currentin);
client.publish("Solar_Node1_IN/currentin", currentin); //Publish message to MQTT Topic Solar_Node1_IN/currentin

static char powerin[7]; 
dtostrf (PZEMPower,6,2,powerin);
client.publish("Solar_Node1_IN/powerin", powerin);     //Publish message to MQTT Topic Solar_Node1_IN/powerin

static char energyin[7]; 
dtostrf (PZEMEnergy,6,2,energyin);
client.publish("Solar_Node1_IN/energyin", energyin);   //Publish message to MQTT Topic Solar_Node1_IN/energyin 
}
}
...
//Publish Soil Moisture Sensor Data to MQTT
  ...
  client.publish("Solar_Node1_IN/soil1", soil1);    //Publish message to MQTT Server to topic soil/soil1
...
//Publish Temperature & Humidity to MQTT
  static char temp[7]; 
  dtostrf (t1,6,2,temp);
  client.publish("Solar_Node1_IN/temp", temp);

  static char humi[7]; 
  dtostrf (h1,6,2,humi);
  client.publish("Solar_Node1_IN/humi", humi); 
